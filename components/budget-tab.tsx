...
import type { Bucket, Recurring } from '@/lib/types'
import { Minus, PiggyBank, Plus, Trash2, TrendingUp } from 'lucide-react'
...
export function BudgetTab({
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* Monthly projection from recurring items */}
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Stuff cash into envelopes toward your goals.
            </p>
          </div>
          <BucketDialog />
        </div>
...
function BucketCard({ bucket }: { bucket: Bucket }) {
  ...
  const stuff = async (delta: number) => {
    setBusy(true)
    try {
      await stuffBucket(bucket.id, delta)
    } catch {
...
function BucketDialog() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await createBucket({ name: name.trim(), target: t, color })
    ...
    >
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Give it a name, a target, and a color.
          </DialogDescription>
        </DialogHeader>
